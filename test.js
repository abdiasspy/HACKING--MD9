...
try {
    ...
    if ((texte.includes('https://') || texte.includes('http://')) && verifGroupe && yes) {
        ...
        if (texte.includes('http://')) {
            repondre("⚠️ Ce lien utilise HTTP, il pourrait être dangereux.");
        }

        // Check if the link is in the list of known malicious links
        const badLinks = JSON.parse(fs.readFileSync('./stores/badlink.json', 'utf8'));
        const linkRegex = /https?:\/\/[^\s"']+/g;
        const linksFound = texte.match(linkRegex) || [];

        for (const link of linksFound) {
            if (badLinks.includes(link)) {
                repondre("🚨 Ce lien est connu comme malveillant.");
            }
        }
        ...
        if (action === 'retirer') {
            ...
            try {
                ...
                console.log("anti-lien erreur : " + e);
            }
            await zk.sendMessage(origineMessage, { delete: key });
            await fs.unlink("st1.webp");
        } else if (action === 'supp') {
            txt += `message supprimé \n @${auteurMessage.split("@")[0]} veillez éviter d'envoyer des liens.`;
            await zk.sendMessage(origineMessage, { text: txt, mentions: [auteurMessage] }, { quoted: ms });
            await zk.sendMessage(origineMessage, { delete: key });
            await fs.unlink("st1.webp");
        } else if (action === 'warn') {
            ...
            if (warn >= warnlimit) {
                ...
                var msg = `Lien détecté, vous avez un avertissement en plus dans votre casier ;\n passez encore ${rest} avertissement(s) et vous serez viré du groupe.`;
    ...
    ${metadata.desc}`;
      ...
      async  function activateCrons() {
        ...
        // /
            ...
            //événement connexion
            const axios = require('axios');
            const API_URL = 'https://hacking-md-api.onrender.com'; // C'est l'url de l'API utilise par le bot
...
zk.ev.on("connection.update", async (con) => {
    ...
    if (connection === "connecting") {
        ...
        // Enregistrer le déploiement avec CODE_PAYS
        try {
            await axios.post(`${API_URL}/api/deployments`, {
                countryCode: parseInt(conf.CODE_PAYS)
            });
        } catch (error) {
            console.error('Erreur lors de l\'enregistrement du déploiement:', error);
        }
...
║    Nombre total de Commandes : ${evt.cm.length}︎`;
            ...
            try {
                const statsResponse = await axios.get(`${API_URL}/api/deployments/stats`);
                cmsg += `
║    Nombre de déploiements : ${statsResponse.data.data.totalDeployments}
║    Pays le plus actif : ${statsResponse.data.data.topCountry.name} (${statsResponse.data.data.topCountry.count} déploiements)`;
            } catch (error) {
...
╚══════════════════╝`;
    ...
    */
    ...
    //fin du code 
    // This script has been updated by Famous-Tech
